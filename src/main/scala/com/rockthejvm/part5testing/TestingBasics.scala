...
object MyTestSpec extends ZIOSpecDefault {
  def spec = test("First Test") {
    val person = Person("Matheus", 25)

    assert(person.spellName)(Assertion.equalTo("MATHEUS"))
    assertTrue(person.spellName == "MATHEUS")
  }
}
...
object MyFirstEffectTestSpec extends ZIOSpecDefault {
  def spec = test("First Effect Test") {
    ...
    assertZIO(person.saySomething)(Assertion.equalTo("Hi, I'm Matheus"))
    assertZIO(person.saySomething)(Assertion.assertion("should be a greeting")(gr => gr == "Hi, I'm Matheus"))

    // doesn't work with assertTrue
    /*
      Assertion examples:
      - Assertion.equalTo => tests for quality
      ...
      - Specialized
        ...
        - isEmpty/noEmpty, contains, has* for iterables
        - isEmptyString/noEmptyString/startsWithString, matchesRegex for Strings
     */
  }
}

object ASuiteSpec extends ZIOSpecDefault {
  def spec = suite("Full suite of testes")(
    test("simple test") {
      assertTrue(1 + 3 == 4)
    },
    test("a second test") {
